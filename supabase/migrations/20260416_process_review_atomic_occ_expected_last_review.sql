-- Keep repo migration state aligned with production:
-- add optimistic concurrency argument p_expected_last_review and stale conflict signal.
...
begin
  ...
  -- 1) Idempotency marker for this review request.
  ...
  -- 2) If duplicate request, return current state.
  ...
  -- 3) Update progress with optimistic concurrency check.
  ...
  -- 4) Apply aggregated study stats update.
  if p_study_delta <> 0 then
